...
# Allow `python -m v2.cli` from repo root
ROOT = Path(__file__).resolve().parents[1]
if str(ROOT.parent) not in sys.path:
    sys.path.insert(0, str(ROOT.parent))

from v2.convert.pipeline import SimlishConverter


def main(argv: list[str] | None = None) -> None:
    ap = argparse.ArgumentParser(description="v2 English→Simlish converter")
