...
from engine.config import ANALYSIS_REPORTS, AUDIO_META, AUDIO_PHASE2, CATALOG_DIR, ensure_dirs
...
def main() -> None:
    ...
    if not (CATALOG_DIR / "songs.json").exists():
        runpy.run_path(str(ROOT / "engine/scripts/01_fetch_official_catalog.py"), run_name="__main__")

    print("\n=== Phase2 resolve YouTube ===")
    sys.argv = [str(ROOT / "engine/scripts/03_resolve_youtube_official.py"), "phase2"]
    runpy.run_path(str(ROOT / "engine/scripts/03_resolve_youtube_official.py"), run_name="__main__")

    print("\n=== Phase2 download audio ===")
    sys.argv = [str(ROOT / "engine/scripts/04_download_audio.py"), "phase2"]
    runpy.run_path(str(ROOT / "engine/scripts/04_download_audio.py"), run_name="__main__")
