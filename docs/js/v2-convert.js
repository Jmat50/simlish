/**
 * Browser port of v2 English→Simlish converter (sound-alike + rhyme + meter + phrase memory).
 * Loads JSON models from ./v2-models/ — independent of the v1 Markov engine.
 */
 ...
 * @returns {string}
 */
export function convertTextV2(text) {
  if (!models) throw new Error("v2 models not loaded");
  if (!text) return "";
...
export async function loadV2Models() {
  ...
  loadPromise = (async () => {
    ...
    const [sa, rhyme, meter, memory, fw, rhymeKeys] = await Promise.all(
      files.map(async (f) => {
        const res = await fetch(`./v2-models/${f}`, { cache: "force-cache" });
        if (!res.ok) throw new Error(`Failed to load v2-models/${f} (${res.status})`);
        return res.json();
      })
    );
    models = {
